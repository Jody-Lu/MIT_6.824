...
func doMap(
	...
	// both the map task number and the reduce task number (see common.go). Use the
	...
	// convert the inFile into slice containing key/value pair
	// dat's type: []byte
	// string(dat): convert []byte to string
	//fmt.Println(key_value_pair[0])
	// key_value_pair[0] is KeyValue type

	content, _ := ioutil.ReadFile(inFile)
	key_value_pair := mapF(inFile, string(content))
